...
- **Cursor provider planning:** Milestone 1 adds ingestion support targeting the default workspace (`~/Library/Application Support/Cursor` on macOS, roaming profile on Windows) with schema-parity fixtures and adapter coverage; settings now surface Cursor paths alongside other providers and server fallbacks probe Cursor directories on macOS/Linux/Windows.
